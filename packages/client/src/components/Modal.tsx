...
export function Modal({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 light:bg-black/30 p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={dialogRef}
        className={`w-full ${width} rounded-lg border border-neutral-700/60 bg-neutral-900/95 shadow-2xl backdrop-blur-md light:border-neutral-300 light:bg-white/95`}
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center justify-between border-b border-neutral-700/30 px-4 py-3 light:border-neutral-200">
          <h2 id={titleId} className="text-sm font-medium text-neutral-200 light:text-neutral-900">
            {title}
          </h2>
          <button
            onClick={onClose}
            className="rounded p-1.5 text-neutral-500 hover:bg-neutral-800 hover:text-neutral-200 light:text-neutral-400 light:hover:bg-neutral-100 light:hover:text-neutral-700"
            title="Close (Esc)"
            aria-label="Close"
          >
            <X size={16} />
    ...
    <Modal open={open} onClose={onClose} title={title} initialFocusRef={inputRef}>
      ...
      >
        <label className="block space-y-1.5">
          <span className="text-xs text-neutral-300 light:text-neutral-600">{label}</span>
          <input
            ref={inputRef}
            type="text"
            value={value}
            ...
            className="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-1.5 text-sm text-neutral-100 outline-none focus:border-neutral-500 light:border-neutral-400 light:bg-white light:text-neutral-900 light:focus:border-neutral-500"
          />
        </label>
        <footer className="flex justify-end gap-2 pt-1">
          <button
            type="button"
            onClick={onClose}
            className="rounded-md border border-neutral-700 px-3 py-1 text-xs text-neutral-200 hover:bg-neutral-800 light:border-neutral-400 light:text-neutral-700 light:hover:bg-neutral-100"
          >
            Cancel
          </button>
          <button
            ...
            className="rounded-md bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-900 hover:bg-white disabled:cursor-not-allowed disabled:opacity-40 light:bg-neutral-800 light:text-neutral-100 light:hover:bg-neutral-700"
          >
            {primaryLabel}
          </button>
        </footer>
  ...
  return (
    <Modal open={open} onClose={onClose} title={title}>
      <div className="flex flex-col gap-3 px-4 py-3">
        <p className="text-xs text-neutral-300 light:text-neutral-600">{message}</p>
        <footer className="flex justify-end gap-2 pt-1">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className={
              tone === "danger"
                ? "rounded-md bg-red-700 px-3 py-1 text-xs font-medium text-red-50 hover:bg-red-600 light:bg-red-600 light:text-white"
                : "rounded-md bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-900 hover:bg-white light:bg-neutral-800 light:text-neutral-100 light:hover:bg-neutral-700"
            }
          >
            {primaryLabel}
          </button>
