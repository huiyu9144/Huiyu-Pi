...
export function App() {
  ...
  // Keep previously-viewed ChatView instances alive in the DOM so
  // switching back to them is instant (CSS display toggle instead
  // of React unmount/remount). Cap at last 5 to prevent unbounded
  // DOM accumulation over long sessions.
  const MAX_CACHED_VIEWS = 10;
  const viewedSessionIds = useRef<string[]>([]);
  if (activeSessionId !== undefined) {
    const arr = viewedSessionIds.current;
    const idx = arr.indexOf(activeSessionId);
    if (idx !== -1) arr.splice(idx, 1);
    arr.push(activeSessionId);
    if (arr.length > MAX_CACHED_VIEWS) {
      arr.splice(0, arr.length - MAX_CACHED_VIEWS);
    }
  }
            ...
            {chatOpen && (
              <div className="flex flex-1 flex-col overflow-hidden">
                {projectsLoaded && projects.length === 0 ? (
                  ...
                  <>
                    {viewedSessionIds.current.map((sid) => (
                      <ChatView key={sid} sessionId={sid} hidden={sid !== activeSessionId} />
                    ))}
